...
Not yet built: Fix-stage persistence (saving/tracking progress on fixes), Monitor/Automate tools, GBP API integration (the Check tool above is self-reported/manual, not API-connected), AI audit engine, review-management automation, security hardening, performance work, final QA, business-type/city pages — each remains its own scoped future session.
